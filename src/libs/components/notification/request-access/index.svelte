...
<div class="absolute w-screen h-screen flex flex-col items-center justify-center">
    <div class="w-[400px] h-[200px] flex shadow-lg divide-y-2 divide-black border-2 border-black flex-col bg-white items-center justify-center">
        ...
        <div class="text-xl h-[100px] font-bold flex divide-x-2 divide-black flex-row items-center justify-center">
            ...
            <button on:click={() => {
                // @ts-ignore
                $ws.emit("decline-access", {id: $page.params.id, requestUserId: $requestAccess.requestId, userConceptId: $concept.user.find((i) => i.userId === $requestAccess.requestId).id})
                requestAccess.set({requestId: "", message: "", show: false})
            }} class="hover:bg-gray-100 transition-all w-[196px] h-full flex flex-col items-center justify-center">DECLINE</button>
        </div>
    </div>
</div>
