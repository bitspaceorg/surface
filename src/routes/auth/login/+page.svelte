<script lang='ts'>
    ...
    const handleLogin = async () => {
        isLoading = true;
        if(!EMAIL || !PASSWORD) notification.set({message: "PLEASE FILL ALL THE FIELDS", type: "ERROR", show: true})
        else{
            try{
                ...
                user.set(res.data)
                notification.set({message: `${$user?.username.toUpperCase()} - LOGGED IN SUCCESSFULLY`, type: "SUCCESS", show: true})
                goto("/")
            }catch(e){
                // @ts-ignore
